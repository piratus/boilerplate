...
function addMiddleware(devServer) {
  ...
  devServer.use(historyApiFallback({
    ...
    disableDotRule: true,
    // For single page apps, we generally want to fallback to /index.html. However we
    // also want to respect `proxy` for API calls. So if `proxy` is specified, we
    // need to decide which fallback to use.
    // We use a heuristic: if request `accept`s text/html, we pick /index.html.
    // Modern browsers include text/html into `accept` header when
    // navigating. However API calls like `fetch()` won’t generally won’t accept
    // text/html. If this heuristic doesn't work well for you, don’t use `proxy`.
  ...
  if (proxy) {
    ...
    devServer.use(
      mayProxy,
      // Pass the scope regex both to Express and to the middleware
      // for proxying of both HTTP and WebSockets to work without
      // false positives.
